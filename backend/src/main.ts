...
async function bootstrap() {
  ...
  const corsOrigin = process.env.NODE_ENV === 'production' ? frontendUrl : [frontendUrl, devAllowedHost];ts];
  ...
  console.log(`🚀 Servidor SIGMUN corriendo en: ${isProduction ? `https://localhost:${port}/api` : `http://localhost:${port}/api`}`);
}
bootstrap();
