...
export default function ModelosPage() {
  ...
  const [data, setData] = useState<ModeloRow[]>([]);
  ...
  const executeSearch = useCallback(
    async (pageNum: number, filtersOverride?: typeof filters) => {
      ...
      setError(null);
      try {
        const result = await searchModelosAction(filtersOverride ?? filters, pageNum, pageSize);
        if (result.success) {
          setData(result.data);
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    setDeleting(true);
    try {
      const res = await eliminarModeloAction(deleteTarget);
      if (res.success) {
      ...
      <div className="relative overflow-hidden rounded-lg bg-gradient-to-br from-sat-navy via-[#1b2b4a] to-slate-800 px-5 py-4 shadow-sm">
        ...
        <div className="relative flex items-center gap-3">
          <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-gradient-to-br from-white/15 to-white/5 backdrop-blur-sm ring-1 ring-white/10">
            <Truck size={18} className="text-white drop-shadow-[0_1px_2px_rgba(0,0,0,0.3)]" />
          </div>
          <div>
