...
export default function FraccionarPapeletaModal({ isOpen, infraccion, onClose, onSuccess }: Props) {
  ...
  const handleGenerarConvenio = async () => {
    ...
    setLoading(true);
    try {
      // Formatear la papeleta/recibo exactamente como en el sistema legacy js_Fraccionarpape.js
      const idreciboVal = infraccion.idRecibo || infraccion.idrecibo || "";
      const codigoVal = infraccion.codigo || infraccion.codigocond || respData.codigo || "";
      const tipoVal = infraccion.tipo || "10.86";
      const tipoRecVal = infraccion.tipoRec || infraccion.tipo_rec || "10.86";
      const montoVal = (infraccion.monto || montoTotalFracc || 0).toString().replace(/,/g, "");
      const placaVal = infraccion.placa || infraccion.codplac || "";
      
      let anioVal = new Date().getFullYear().toString();
      if (infraccion.fecha && infraccion.fecha.length >= 4) {
        // extrae el año si viene en formato YYYY-MM-DD o DD/MM/YYYY
        const parts = infraccion.fecha.split(/[-/]/);
        if (parts.length === 3) {
          anioVal = parts[0].length === 4 ? parts[0] : parts[2];
        }
      }
